...
class OneFrameCachGlData {
    addCachGlDatas(v: CachGlData) {
        this.cachGlDatas.push(v)
    }
...
let renderZipData = new win.JSZip()
function writeGlRenderData() {
    renderZipData.generateAsync({ type: "blob" }).then((content: any) => {
        // file-saver保存文件
        fileSaveAs(content, `renderData.zip`)
    })
    renderZipData = new win.JSZip()
}
...
let count = 0
function replaceWebglFunc(gl: any) {
                ...
                if (debugCpuRender) {
                    ...
                    if (createTsImplGlslFile) {
                        ...
                        if (funcKey == "shaderSource") {
                            ...
                            if (hash === "b7ee481f59661c68ac91aae453fcaa0d") {
                                debugger
                                console.log("hash: b7ee481f59661c68ac91aae453fcaa0d content:")
                                console.log(shaderSource)

                                // 使用自己预编译的代码 看是否是正确的
                                shaderSource = `#define CC_PIPELINE_TYPE 0
...
export class CpuRenderingContext {
    constructor() {
        // wait todo
        // let anyGame = <any>game
        // let device = anyGame._gfxDevice
    ...
    customContextInit(sp: HTMLCanvasElement): void {
        ...
        //     () => {
        //         // renderZipData.file(Date.now(), JSON.stringify(nowFrameCachData))
        //         // nowFrameCachData = new OneFrameCachGlData()
        //         // frames++
        //         // if (frames > 60 * 10) {
        //         //     frames = 0
        //         //     writeGlRenderData()
